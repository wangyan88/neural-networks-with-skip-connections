...
class dataProcess(object):
	...
	def __init__(self, out_rows, out_cols, data_path = "../deform/train", label_path = "../deform/label", test_path = "../test",test_label_path='C:/Users/ppk/Code/GAN/Test_lab', npy_path = "../npydata", img_type = "tif"):
	...
	def create_train_data(self):
		...
		print(len(imgs))
		imgdatas = np.ndarray((len(imgs)*5,self.out_rows,self.out_cols,1), dtype=np.uint8)
		imglabels = np.ndarray((len(imgs)*5,self.out_rows,self.out_cols,1), dtype=np.uint8)
		for imgname in imgs:
#			print(imgname)
...
#			img = load_img(self.data_path + '/' + 'train-volume.'+self.img_type,grayscale = True)
#			label = load_img(self.label_path + '/' + 'train-labels.'+self.img_type,grayscale = True)
			...
			Temp=random_rotation(img_label,90,row_axis=0, col_axis=1, channel_axis=2)
#			Temp2=random_rotation(img_label,180,row_axis=0, col_axis=1, channel_axis=2)
#			Temp3=random_rotation(img_label,270,row_axis=0, col_axis=1, channel_axis=2)
			...
			imglabels[i+3] = temp			
			'''
			Temp=random_shift(img_label,wrg=0.2,hrg=0.2,row_axis=0, col_axis=1, channel_axis=2)
			temp=[]
			temp=(Temp[:,:,0])
			temp.resize(128,128,1)
			imgdatas[i+2] = temp						
...
#		print((imgs))
		...
		Name = ['a' for i in range(len(imgs))]
		i = 0

		for imgname in imgs:
			midname = imgname[imgname.rindex("\\")+1:]
			Name[i] = midname[:midname.rindex('.')]
			Index = (re.search(r'[0-9]', Name[i]).span())[0]
			Name[i] = int(Name[i][Index:])
#			print(Name[i])
#			print('test: ',midname)
			img = load_img(self.test_path + "/" + midname,grayscale = True)
			label = load_img(self.test_label_path + "/" + midname,grayscale = True)			
#			img = load_img(self.test_path+'/'+'test-volume.'+self.img_type,grayscale = True)
			img = img_to_array(img)
			label = img_to_array(label)
		...
		np.save(self.npy_path + '/name.npy', Name)
...
#		mean = imgs_train.mean(axis = 0)
		for i in range(imgs_train.shape[0]):
			mean = imgs_train[i,:,:,0].mean()
			temp = np.ones((128,128,1)) * mean
			imgs_train[i,:,:,:] -= temp
#		print('train_mean: ',imgs_train.shape[0])
#		imgs_train -= mean	
		imgs_mask_train /= 255
	...
	def load_test_data(self):
		...
		Name = np.load(self.npy_path+'/name.npy')
		Name=Name.tolist()
		
		imgs_test /= 255
		for i in range(imgs_test.shape[0]):
			mean = imgs_test[i,:,:,0].mean()
			temp = np.ones((128,128,1)) * mean
			imgs_test[i,:,:,:] -= temp	
		...
		return imgs_test,imgs_mask_test,Name
